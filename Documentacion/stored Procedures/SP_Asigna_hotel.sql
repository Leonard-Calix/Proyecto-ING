...
drop procedure if exists SP_ASIGNA_HOTEL;

delimiter $$
create procedure SP_ASIGNA_HOTEL(in _idHotel int,
                                    in _idTours int, 
...
begin
	...
	if exists (select * from Hotel where idHotel = _idHotel) then
		...
		if not exists (select idtours from hotel where idtours = _idtours) then 
			set _mensaje = 1;
